...
部分页面会保留只有 category、但数据单元格完全为空的可选模板行，例如初月的“舰船型号”。这类行不包含文本、链接、媒体或 `ship_word_block`，因此安全忽略；只要数据单元格非空却缺少语音块，解析器仍严格报错。
...
精确匹配先做 NFC Unicode 规范化、去除输入首尾空白和 `casefold`。本地规范名未命中时，再调用 MediaWiki `query&redirects=1&titles=...`；只有重定向目标也属于 `Category:舰娘` 才会自动采用。仍未命中时，站内全文搜索结果会与舰娘分类取交集，并优先加入候选；本地模糊建议再按前缀、包含关系和字符串相似度补足。非权威候选只供用户选择，不会自动执行。
...
- HTTP 429、500、502、503、504、567。`567` 是 BWiki/CDN 实际出现过的临时服务端状态。
...
- `hatsuzuki.html`：本体空“舰船型号”可选行、含特殊符号的皮肤名；解析为 4 个语音集、82 条台词、82 条音频。
