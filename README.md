...
- 当前版本：`v2.0.0`。第二版新增展示名/重定向名称查找、分类过滤的站内候选，并修复 BWiki `567` 临时错误的自动重试。
- 交互式指定舰娘和全部舰娘模式均已实现。
...
测试完全使用本地 fixture，不会在每次测试时访问 BWiki。当前真实样本包括欧根亲王、标枪和曾克海军上将，覆盖多皮肤、少皮肤、无音频条目、彩蛋嵌套标签、展示别名和没有“皮肤描述”的誓约皮肤。
